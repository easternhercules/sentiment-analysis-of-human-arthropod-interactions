...
```{r read}
#read
obs_interest <- st_read(
  "C:/Users/Julian/Downloads/R_files_distribution/inat/observations-All.shp")
obs_all <- read.csv("C:/Users/Julian/Downloads/R_files_distribution/inat/full/observations-Full.csv")
...
col_order <- c("stateinUSA", "obscount", "totalobscount", "normcount")
obsperstate_joined <- obsperstate_joined[, col_order]
obsperstate_joined$normcount <- trunc(obsperstate_joined$normcount)
obsperstate_joined <- obsperstate_joined[order(-obsperstate_joined$normcount),]
remove(col_order)
write.csv(obsperstate_joined,"C:/Users/Julian/Downloads/R_files_distribution/obsperstate_joined.csv", row.names = FALSE)
```
...
usa_basemap@data$groups <- cut(usa_basemap@data$normcount, breaks=c(-1,0,2,5,9,15,25), labels=c("0","1-2","3-5","6-9","10-15",">16"))
#convert to df for ggplot
usa_basemap@data$id = rownames(usa_basemap@data)
usa_basemap.points = fortify(usa_basemap, region="id")
usa_basemap.df = join(usa_basemap.points, usa_basemap@data, by="id")
...
map_obsperstatenorm <- ggplot(usa_basemap.df) +  
  ...
  labs(x = NULL, 
         y = NULL, 
         title = "iNaturalist research-grade observations of medically-important spider genera (normalized)", 
         subtitle = "(December 31 1979-February 18 2019)", 
         caption = "Available from https://www.inaturalist.org. Accessed February 18 2019") +
  coord_sf(crs = st_crs(WGS84_proj), xlim=c(-124.7, -67.1), ylim = c(25.2, 49.4)) + annotation_north_arrow(style = north_arrow_fancy_orienteering, location = "tr", which_north = "true") + 
  scale_size_continuous(range = c(1, 8), breaks = c(250, 500, 750, 1000)) + 
...
map_obsperstatenorm
#export dataframes
write.csv(usa_basemap.df,"C:/Users/Julian/Downloads/R_files_distribution/map_obsperstatenorm.csv", row.names = FALSE)
```
