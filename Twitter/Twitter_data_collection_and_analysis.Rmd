...
ggplot(posnegtweets, aes(factor(type), count, fill = sentiment)) + geom_bar(stat="identity", position = "dodge") + 
  scale_fill_brewer(palette = "Set1") +
      ...
      annotate("text", x = 2, y = 7000, label = "likes: p = 5.76e-05, mean = 6.1415") +
      annotate("text", x = 2, y = 6500, label = "retweets: p = 0.01828, mean = 0.6535")
...
histogram <- ggplot(data=spider_tweets_sentiment@data, aes(spider_tweets_sentiment@data$stringsentiment)) +
    ...
    annotate("text", x = 5, y = 20500, label = "mean = -0.1422473")
histogram
```
...
usa_basemap@data$groups <- cut(usa_basemap@data$statesentiment, breaks=c(-0.5,-0.31,-0.21,-0.11,-0.01,0.10), labels=c("<-0.31","-0.3 - -0.21","-0.2 - -0.11","-0.1 - -0.01","0=<"))
...
colors <- colors[-7]
map_statesentiment <- ggplot(usa_basemap.df) +  
...
plottweetsandpop <- ggplot(tweetsandpop, aes(x = normcount, y = estpop)) + 
  ...
  annotate("text", x = 35, y = 2e+07, label = "mean(x) = 12.829")
