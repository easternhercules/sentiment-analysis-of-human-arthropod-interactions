...
posneg
#statistics
posneg.aov <- aov(formula = n ~ Sentiment,
                   data = posneg[["data"]])
summary(posneg.aov)
mean(posneg[["data"]]$n)
```
...
ggplot(posnegtweets, aes(factor(type), count, fill = sentiment)) + geom_bar(stat="identity", position = "dodge") + 
  scale_fill_brewer(palette = "Set1") +
      ...
      annotate("text", x = 2, y = 7000, label = "likes: t = 4.03, p = 5.8e-05, mean = 6.14") +
      annotate("text", x = 2, y = 6500, label = "retweets: t = 2.36, p = 0.02, mean = 0.65")
...
histogram <- ggplot(data=spider_tweets_sentiment@data, aes(spider_tweets_sentiment@data$stringsentiment)) +
    ...
    annotate("text", x = 5, y = 24500, label = "t = -29") +
...
#export
writeOGR(obj=usa_basemap, dsn=here::here("data", "meanchangesentimentshp"), layer="usa_basemap", driver="ESRI Shapefile")
...
colors[4] <- "gray85"
#map change between overall and state means
...
plottweetsandpop <- ggplot(tweetsandpop, aes(x = normcount, y = estpop)) + 
  ...
  annotate("text", x = 35, y = 4.3e+07, label = "f = 1.31") +
